...
def train_saivrNet(model, optimizer, scheduler, t_0, t_final, initial_conditions_set, parameters_bundle, parameters_fixed, epochs,
    ...
    # alphas, betas, gammas, deltas = parameters_bundle[0][:], parameters_bundle[1][:], parameters_bundle[2][:], parameters_bundle[3][:]
    betas, gammas, deltas = parameters_bundle[0][:], parameters_bundle[1][:], parameters_bundle[2][:]
   ...
   # Train mode
    ...
    for epoch in tqdm(range(epochs), desc='Training'):
        ...
        for i, t in enumerate(t_dataloader, 0):
            ...
            # alpha = uniform(alphas[0], alphas[1], size=batch_size)
            ...
            # param_bundle = [alpha, beta, gamma, delta]
            param_bundle = [beta, gamma, delta]
            ...
            batch_loss = sir_loss(t, s, i, v, r, param_bundle, parameters_fixed, decay, weight_i_loss)

            # Hacking the system to prevent the ODEs to be solved trivially
            if hack_trivial:
                # batch_trivial_loss = trivial_loss(i, a, v, hack_trivial)
                batch_trivial_loss = trivial_loss(i, v, hack_trivial)
        ...
        if train_epoch_loss < loss_threshold:
            # model_scripted = torch.jit.script(best_model, optimize=optimizer)
            # model_scripted.save('Unsupervised_1.pt')
            # Path = ROOT_DIR + '\trained_models\{}'.format(model_name)
            Path = r"C:\Users\user1\Documents\2021_2022\Covid19_SIR\NN_approach\trained_models_1"
            # torch.save(best_model, Path)
            torch.save({'model_state_dict': best_model.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
                        'loss': train_epoch_loss},Path + '/Unsupervised_1.pt')
                        # Path)
            break
            # torch.save({'model_state_dict': best_model.state_dict(),
            #             'optimizer_state_dict': optimizer.state_dict(),
            #             'loss': train_epoch_loss},
            #             # ROOT_DIR + '\trained_models\{}'.format(model_name))
            #             ROOT_DIR + '\trained_models\Unsupervised_1.')
        ...
        if epoch%250 == 0:
           ...
           test_snippet(model, epoch, train_epoch_loss, t_0, t_final, parameters_fixed, i0=i_0[0], v0=v_0[0], 
                                                                        #    r0=r_0[0], alpha_1 = alpha[0], 
                                                                           r0=r_0[0], 
                                                                           beta_1 = beta[0], gamma = gamma[0],
                                                                           delta = delta[0])
...
def fit_data_synthetic(model, time_series_dict, initial_conditions_set, parameters_bundle, param_fixed, lr, epochs,
    ...
    print('\n Initial (random) parameters \n' 'S0 = {:.2f}, I0 = {:.2e}, V0 = {:.2f}, R0 = {:.2f} \n'
          'Alpha_1 = {:.2f}, Beta_1 = {:.2f}, $\gamma$ = {:.2f}, $\delta$ = {:.2e} \n'.format(s_0.item(), i_0.item(),
    ...
    optimizer = torch.optim.Adam([i_0, v_0, r_0, alpha_1, beta_1, gamma, delta], lr=lr)    
    ...
    for epoch in tqdm(range(epochs), desc='Finding the best parameters/initial conditions'):  
        ...
        for t in time_sequence:
            ...
            s_hat, i_hat, v_hat, r_hat = model.parametric_solution(t_tensor, t_0,
                                                                          initial_conditions,
                                                                               param_bundle)
            
            # losses = data_fitting_loss(t_tensor, true_pop, s_hat, a_hat, i_hat, v_hat, r_hat)
            losses = data_fitting_loss(t_tensor, true_pop, s_hat, i_hat, v_hat, r_hat)            
        ...
        if epoch%250 == 0 and epoch != 0:
            ...
            print('Fitted parameters \n' 'S0 = {:.2f}, I0 = {:.2e}, V0 = {:.2f}, R0 = {:.2f} \n'
                                    ...
                                    #    .format(s_0.item(), a_0.item(), i_0.item(), v_0.item(),
                                       .format(s_0.item(), i_0.item(), v_0.item(),
    ...
    optimized_params = [ s_0.item(), i_0.item(), v_0.item(),    
                        r_0.item(), alpha_1.item(), beta_1.item(), gamma.item(), delta.item()]       
...
def fit_data_real(model, time_series_dict, initial_conditions_set, parameters_bundle, param_fixed, lr, epochs,
    ...
    print('\n Initial parameters \n' 'S0 = {:.2f}, I0 = {:.2e}, V0 = {:.2e}, R0 = {:.2f} \n'
        #   'Alpha_1 = {:.2f}, Beta_1 = {:.2f}, $\gamma$ = {:.2f}, $\delta$ = {:.2f} \n'.format(s_0.item(), i_0.item(),
          'Beta_1 = {:.2f}, $\gamma$ = {:.2f}, $\delta$ = {:.2f} \n'.format(s_0.item(), i_0.item(),
                                    #  v_0.item(), r_0.item(), alpha_1.item(), beta_1.item(), gamma.item(), delta.item()))
                                     v_0.item(), r_0.item(), beta_1.item(), gamma.item(), delta.item()))

    learning_rate_dicts = [ {'params' : [i_0, v_0], 'lr' : 1e-5},
                            # {'params' : [alpha_1, beta_1, gamma, r_0, a_0], 'lr' : 1e-1},
                            {'params' : [beta_1, gamma, r_0], 'lr' : 1e-1},
    ...
    for epoch in tqdm(range(epochs), desc='Finding the best parameters/initial conditions'):  
        ...
        for t in time_sequence:
            ...
            # loss_i = data_fitting_loss(t_tensor, true_pop, s_hat, i_hat, v_hat, r_hat, mode = 'mse', data_type = 'real')
            loss_i = data_fitting_loss(t_tensor, true_pop, s_hat, i_hat, v_hat, r_hat, mode = 'mse', data_type = 'real')
            ...
            regularization = torch.exp(-0.005 * i_0)
        ...
        # optimized_params = s_0.item(), i_0.item(), v_0.item(), r_0.item(), alpha_1.item(), beta_1.item(), gamma.item(), delta.item()
        optimized_params = s_0.item(), i_0.item(), v_0.item(), r_0.item(), beta_1.item(), gamma.item(), delta.item()
        ...
        if epoch%250 == 0 and epoch != 0:
            ...
            print('Fitted parameters \n' 'S0 = {:.2f}, I0 = {:.2e}, V0 = {:.2e}, R0 = {:.2f} \n'
                                    ...
                                    #    .format(s_0.item(), a_0.item(), i_0.item(), v_0.item(),
                                        .format(s_0.item(), i_0.item(), v_0.item(),                            
                                        r_0.item(), beta_1.item(), gamma.item(), delta.item()))
                                        # r_0.item(), alpha_1.item(), beta_1.item(), gamma.item(), delta.item()))
            ...
            torch.save({'model_state_dict': model.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
                        'loss': epoch_loss}, Path + '/Supervised_2.pt')
